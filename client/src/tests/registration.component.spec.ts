...
it('should have specialty and availability as optional for PATIENT role', () => {
  component.itemForm.controls['role'].setValue('PATIENT');
  component.itemForm.controls['email'].setValue('test@test.com');
  component.itemForm.controls['password'].setValue('password');
  component.itemForm.controls['username'].setValue('testuser');
  expect(component.itemForm.valid).toBeTrue();
});

it('should have specialty and availability as optional for RECEPTIONIST role', () => {
  component.itemForm.controls['role'].setValue('RECEPTIONIST');
  component.itemForm.controls['email'].setValue('test@test.com');
  component.itemForm.controls['password'].setValue('password');
  component.itemForm.controls['username'].setValue('testuser');
  expect(component.itemForm.valid).toBeTrue();
...
// registration.component.spec.ts — add 2 more
it('should initialize responseMessage as undefined', () => {
  expect(component.responseMessage).toBeUndefined();
});

it('should have formModel with null role initially', () => {
  expect(component.formModel.role).toBeNull();
});
});
