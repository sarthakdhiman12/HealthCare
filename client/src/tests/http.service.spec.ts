...
it('should get medical records by patient', () => {
  const patientId = 1;
  const mockResponse = [
    { id: 1, diagnosis: 'Fever', treatment: 'Rest' },
    { id: 2, diagnosis: 'Cold', treatment: 'Medicine' }
  ];

  service.getMedicalRecords(patientId).subscribe((response: any) => {
    expect(response).toEqual(mockResponse);
  });

  const req = httpMock.expectOne(
    `${service.serverName}/api/patient/medicalrecords?patientId=${patientId}`
  );
  expect(req.request.method).toBe('GET');
  expect(req.request.headers.get('Authorization')).toBe('Bearer mockToken');
  req.flush(mockResponse);
...
it('should send POST request with correct content type for login', () => {
  const loginDetails = { username: 'user1', password: 'pass1' };
...
it('should send POST request with correct content type for register patient', () => {
  const details = { username: 'patient', email: 'patient@test.com', password: 'pass' };
  service.registerPatient(details).subscribe();
  const req = httpMock.expectOne(`${service.serverName}/api/patient/register`);
  expect(req.request.headers.get('Content-Type')).toBe('application/json');
  req.flush({});
});

it('should send POST request with correct content type for register doctor', () => {
  const details = { username: 'doctor', email: 'doctor@test.com', password: 'pass' };
  service.registerDoctors(details).subscribe();
  const req = httpMock.expectOne(`${service.serverName}/api/doctors/register`);
  expect(req.request.headers.get('Content-Type')).toBe('application/json');
  req.flush({});
});

it('should send POST request with correct content type for register receptionist', () => {
  const details = { username: 'receptionist', email: 'rec@test.com', password: 'pass' };
  service.registerReceptionist(details).subscribe();
  const req = httpMock.expectOne(`${service.serverName}/api/receptionist/register`);
  expect(req.request.headers.get('Content-Type')).toBe('application/json');
  req.flush({});
});

it('should send Authorization header when getting doctors', () => {
...
it('should send Authorization header when getting appointments by doctor', () => {
  service.getAppointmentByDoctor(1).subscribe();
  const req = httpMock.expectOne(`${service.serverName}/api/doctor/appointments?doctorId=1`);
  expect(req.request.headers.get('Authorization')).toBe('Bearer mockToken');
  req.flush([]);
});
